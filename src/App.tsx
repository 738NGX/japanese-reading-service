  ...
  return <main>
    ...
    <section className="tool">
      <form onSubmit={submit}>
        ...
        <fieldset><legend>用途</legend>{modes.map((item) => <label className="mode" key={item.value}><input type="radio" name="mode" checked={mode === item.value} onChange={() => setMode(item.value)} />{item.label}</label>)}</fieldset>
      </form>
      {error ? <p className="error" role="alert">{error}</p> : null}
      {result ? <Results result={result} selectedKeys={selectedKeys} onSelect={(index, candidate) => setSelectedKeys((current) => ({ ...current, [index]: candidateKey(candidate) }))} /> : <p className="hint">使用 OpenCC 实时转换为日文新字体；整词词典命中会优先显示。</p>}
    </section>
    <footer>候选仅供辅助判断。地名、人名等专名请以权威词典或官方资料为准。</footer>
  </main>;
}

function Results({ result, selectedKeys, onSelect }: { result: ApiResult; selectedKeys: Record<number, string>; onSelect: (index: number, candidate: KanjiReadingCandidate) => void }) {
  const selectedCandidates = result.characters.map((character, index) => character.candidates.find((candidate) => candidateKey(candidate) === selectedKeys[index]) ?? character.selected ?? character.candidates[0]).filter((candidate): candidate is KanjiReadingCandidate => Boolean(candidate));
  ...
  return <section className="results" aria-live="polite">
    ...
    <h2>推荐候选</h2>
    <div className="candidate-list">{result.combinationCandidates.slice(0, 5).map((candidate, index) => <article className={index === 0 ? 'candidate primary' : 'candidate'} key={`${candidate.source}-${candidate.reading}`}><span>{index === 0 ? '优先' : candidate.source === 'dictionary' ? '词典' : '推测'}</span><strong>{candidate.reading}</strong><small>{candidate.romaji}</small><p>{candidate.notes.join(' · ')}</p></article>)}</div>
    <h2>逐字候选结果</h2>
